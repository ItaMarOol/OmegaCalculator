"""This module handling the calculator testing."""
import pytest
...
def test_complex_expression2():
    assert (
        calculator.calculate("(10 + 4 * 3) $ (7 - 5) & ((2 ^ 3) + 4) * (6 % 4) - 10")
        == 14
    )
...
def test_complex_expression6():
    assert (
        calculator.calculate(
            "5 * (8 + 3^2) - 10 % 3 + 12 @ 6 * 7 - (15 & 10) + (4 $ 9) - 7!"
        )
        == -4894
    )
...
def test_complex_expression13():
    assert (
        calculator.calculate("((6 + 2) * (4 - 1)) - (4 ^ (2 + 1)) + (8 & 3) @ 2")
        == -37.5
    )
